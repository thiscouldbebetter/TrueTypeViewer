...
class CmapEncodingTable
{
	...
	charCodeToGlyphIndexLookup: number[];
	...
	static fromBytes(reader: ByteStreamBigEndian, length: number): CmapEncodingTable[]
	{
		// See:
		...
		for (var e = 0; e < numberOfEncodingTables; e++)
		{
			...
			else if (formatCode == 4)
			{
				...
				var charCodeToGlyphIndexLookup: number[] = [];
	...
	toBytes(): number[]
	{
		...
		else if (formatCode == 4)
		{
			...
			var segmentEndCharCodes: number[] = [];
			for (var s = 0; s < segmentCount; s++)
			{
			...
			var segmentStartCharCodes: number[] = [];
			for (var s = 0; s < segmentCount; s++)
			{
			...
			var idDeltasForCharCodesInSegments: number[] = [];
			for (var s = 0; s < segmentCount; s++)
			{
				var idDeltaForCharCodesInSegment =
					idDeltasForCharCodesInSegments[s];
				writer.writeShortSigned(idDeltaForCharCodesInSegment);
			}

			var addressesOfIdRangeOffsetsForSegment: number[] = [];
			var idRangeOffsetsInBytesForSegments: number[] = [];
			for (var s = 0; s < segmentCount; s++)
			{
				var addressOfIdRangeOffsetForSegment =
					addressesOfIdRangeOffsetsForSegment[s];
				writer.byteIndexCurrent = addressOfIdRangeOffsetForSegment; // This is not ideal.

				var idRangeOffsetInBytesForSegment =
					idRangeOffsetsInBytesForSegments[s];
