...
class FontTrueType
{
	...
	encodingTables: CmapEncodingTable[];
	headerTable: HeaderTable;
	maximumProfile: MaximumProfile;
	indexToLocationTable: LocationTable;

	glyphsByOffsetInBytes: Map<number, Glyph>;

	constructor(name: string, glyphs: Glyph[])
	{
		this.name = name;
		this.glyphs = glyphs;
	}

	glyphByOffset(offset: number): Glyph
	{
	...
	drawToDisplay(display: Display, fontHeightInPixels: number): void
	{
	...
	drawToDisplay_GlyphBackground
	(
		display: Display,
		fontHeightInPixels: number,
		baseLineOffset: Coords,
		drawOffset: Coords
	)
	{
		var colorBack = "White";
	...
	fromBytes(bytesFromFileAsBinaryString: string): FontTrueType
	{
	...
	fromBytes_ReadTables(reader: ByteStreamBigEndian): void
	{
		// offset table
		...
		for (var t = 0; t < tableNamesOrderedLogically.length; t++)
		{
			...
			{
				this.glyphs = GlyphSimple.tableFromBytes
				(
					reader, tableDefn.length
				);

				this.glyphsByOffsetInBytes =
					new Map<number, Glyph>();
				for (var g = 0; g < this.glyphs.length; g++)
				{
					var glyph = this.glyphs[g];
	...
	toBytes_WriteTables(writer: ByteStreamBigEndian): void
	{
		// offset table
	...
	toDomElement(): any
	{
		...
		var inputFontHeightInPixels: any = d.getElementById("inputFontHeightInPixels");
